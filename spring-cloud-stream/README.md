...
When running the native image of `producer--cloud-stream` and `consumer--cloud-stream`, the containers don't connect to Kafka. The `KAFKA_URL` environment variable informed (that is `kafka:9092`) is not replaced correctly as the application is trying to connect to `localhost/127.0.0.1:9092`.
```
